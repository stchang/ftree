...
(define dqR (build-raseqR size (λ (x) x)))
(check-true
 (for/and ([thresh size])
   (= (sumR thresh dqL)
      (for/sum ([n thresh]) n))))
...
(define dqR2 (build-raseqR2 size (λ (x) x)))
(check-true
 (for/and ([thresh size])
   (= (sumR2 thresh dqL2)
      (for/sum ([n thresh]) n))))
...
(define dqL/2a (build-raseqL size/2 (λ (x) x)))
(define dqL/2b (build-raseqL size/2 (λ (x) (+ x size/2))))
(check-true
...
(define dqR/2a (build-raseqR size/2 (λ (x) x)))
(define dqR/2b (build-raseqR size/2 (λ (x) (+ x size/2))))
(check-true
 (for/and ([thresh size/2])
   (= (sumL2 thresh dqR/2a)
      (for/sum ([n thresh]) n))))
