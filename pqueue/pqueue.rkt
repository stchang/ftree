...
(provide mk-pqueue pqueue? pq-empty? 
         ...
         pq-append pq-top+rest)
(struct ftree:pqueue ftree (cmp))

(define (mk-pqueue inf cmp)
  (match-define (ftree ∅ sz ⊕ FT) (mk-ftree inf (λ (x) x) (λ (x y) (if (cmp x y) x y))))
  (ftree:pqueue ∅ sz ⊕ FT cmp))
...
(define (pq-top+rest pq)
  (match-define (ftree:pqueue ∅ sz ⊕ FT cmp) pq)
  (match/values (FT-split-tree (λ (x) (not (cmp (sz FT) x))) ∅ sz ⊕ FT)
    [(l x r) (values x (ftree:pqueue ∅ sz ⊕ (FT-append sz ⊕ l r) cmp))]))